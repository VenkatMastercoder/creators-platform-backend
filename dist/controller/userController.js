...
const userIdUpdateController = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, bio, image, socialMediaLinks, username, hashtags } = req.body;
        const data = await userService.updateUsers(id, name, bio, image, socialMediaLinks, username, hashtags);
        if (data.success) {
